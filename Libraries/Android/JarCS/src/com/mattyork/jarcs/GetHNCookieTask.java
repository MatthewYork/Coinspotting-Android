...
public class GetHNCookieTask extends AsyncTask<Void, Void, Void> {
	...
	protected Void doInBackground(Void... arg0) {
		...
		try {
			//Make the request
			HttpResponse response = CSManager.getInstance().Service.client.execute(httpGet);
			
			//Get cookie
			List<Cookie> cookies = CSManager.getInstance().Service.client.getCookieStore().getCookies();
			if (cookies.size() > 0) {
